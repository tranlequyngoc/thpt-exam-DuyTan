...
# AUTO-SEED: Tao de thi tu dong khi khong co de nao
try:
    _c = get_db()
    _cnt = _c.execute("SELECT count(*) as c FROM exams").fetchone()['c']
    if _cnt == 0:
        print("[AUTO-SEED] Khong co de thi, dang tu dong tao...")
        _admin = _c.execute("SELECT id FROM users WHERE role='admin' ORDER BY id LIMIT 1").fetchone()
        _aid = _admin['id'] if _admin else 1
        _subs = {
            'Toan':{'t':90,'q':[('Ham so y=x^3-3x^2+2 bao nhieu cuc tri?','0','1','2','3','C'),('Tap XD y=log2(x-1):','(1;+inf)','[1;+inf)','(0;+inf)','(-inf;1)','A'),('Nguyen ham f(x)=2x+1:','x^2+x+C','x^2+C','2x^2+x+C','x+C','A'),('Tich phan 2f(x)dx(0->1) khi f=3:','3','6','9','12','B'),('z=3+4i modun:','3','4','5','7','C'),('2^x=8 x=?','2','3','4','8','B'),('V chop S.ABCD vuong a SA=a:','a3/3','a3/6','a3','a3/2','A'),('KC O den x+2y-2z+3=0:','1','2','3','3/5','A'),('CSC u1=2 d=3 u10=?','27','29','30','32','B'),('log2(3)=a log2(12)=?','a+2','2a','3a','a+1','A'),('y=(x+1)/(x-2) TCDung:','x=-1','x=1','x=2','x=-2','C'),('DT mat cau R:','2piR2','4piR2','piR2','3piR2','B'),('a(1;2;-1) b(2;-1;3) a.b=?','-3','3','-1','1','A'),('x2+y2=9 R=?','9','3','sqrt9','81','B'),('y=x4-2x2+1 dong bien:','(-1;0)','(0;1)','(-1;1)','(1;+inf)','D')]},
            'Vat ly':{'t':50,'q':[('Don vi cuong do dong dien:','Volt','Ampe','Ohm','Watt','B'),('Coulomb F=kq1q2/r^?','1','2','3','4','B'),('Van toc song DT chan khong:','3e6','3e8','340','3e5','B'),('Quang dien la:','e but ra khi AS','Phat sang','Nhieu xa','Phan xa','A'),('f dao dong cuong buc=:','f rieng','f bat ky','f luc cuong buc','0','C'),('T con lac don theo:','A,m','l,g','m,g','A,l','B'),('PU hat nhan toa nhiet:','HK duong','HK am','Khong HK','m tang','A'),('RLC cong huong:','R=0','ZL=ZC','ZL>ZC','ZL<ZC','B'),('AS buoc song ngan nhat:','Do','Vang','Luc','Tim','D'),('hf=A+Wd A la:','Dong nang','Cong thoat','NL photon','The nang','B'),('x=5cos(2pit) Bien do:','2cm','5cm','10cm','2pi','B'),('Song co truyen:','Chan khong','MT vat chat','Ca hai','Ran','B'),('Tu cam khi:','I ko doi','I bien thien','I=0','q yen','B'),('Tia X buoc song:','Lon hon AS','Ngan hon AS','Bang AS','KXD','B'),('Lorentz phuong:','Cung v','Cung B','Vuong goc v,B','Bat ky','C')]},
            'Tieng Anh':{'t':60,'q':[('She_to school daily','go','goes','going','gone','B'),('If I_you Id study harder','am','was','were','be','C'),('Book_by millions','has read','has been read','have been read','is reading','B'),('Man_car stolen','who','whom','whose','which','C'),('Different:thought/though/through','thought','though','through','thorough','B'),('Movie_when arrived','started','has started','had started','was starting','C'),('_raining we went','Because','Although','Since','When','B'),('Syn of abundant:','rare','scarce','plentiful','limited','C'),('Suggested_cinema','go','to go','going','went','C'),('Wish I_French','can','could','will','may','B'),('Worked here_2020','for','since','in','from','B'),('Ant of optimistic:','hopeful','confident','pessimistic','cheerful','C'),('Not until arrived_keys','she realized','did she realize','she did','realized she','B'),('Children_play park','use to','used to','are used to','using to','B'),('People say genius=','said to be','said to be','is say','was said be','A')]},
            'Hoa hoc':{'t':50,'q':[('DP la QT OxH khi dong:','2 chieu','1 chieu','Xoay chieu','Khong can','B'),('Anion DP nong chay ve:','Anode OxH','Anode khu','Cathode khu','Cathode OxH','A'),('DP NaCl nc anode:','OxH Cl-','OxH Na+','Khu Cl-','Khu Na+','A'),('SP DP nc MgCl2:','MgO+HCl','Mg+H2+Cl2','MgCl2+H2O','Mg+Cl2','D'),('CT glucose:','C12H22O11','C6H12O6','(C6H10O5)n','C2H5OH','B'),('Polime thien nhien:','PE','PVC','Cao su buna','Tinh bot','D'),('Amino axit:','Axit','Bazo','Luong tinh','Trung tinh','C'),('Amin bac 1:','CH3NH2','(CH3)2NH','(CH3)3N','C6H5NHCH3','A'),('DP NaCl mang ngan:','Na+Cl2','NaOH+Cl2+H2','NaCl+H2O','Na+H2','B'),('Fe+HCl:','FeCl3+H2','FeCl2+H2','Fe2O3+H2','FeCl2+H2O','B'),('Nuoc cung tam thoi:','Ca,Mg,Cl','Ca,Mg,HCO3','Na,K,SO4','Fe,Cu','B'),('Chat beo este:','Glix+abeo','Etanol+abeo','Metanol+axit','Glix+HCl','A'),('Phenol tinh:','Axit yeu','Bazo manh','Trung tinh','Axit manh','A'),('TF saccarozo:','Glu','Fruc','Glu+Fruc','Tinh bot','C'),('Khi nha kinh:','O2','N2','CO2','H2','C')]},
            'Ngu van':{'t':120,'q':[('Vo chong A Phu:','To Hoai','Kim Lan','NMChau','NTuan','A'),('Vi Hanh but phap:','Tuong trung','Chau','Nhan hoa','So sanh','B'),('Tay Tien tho:','Quang Dung','To Huu','CLVien','Huy Can','A'),('NV Chiec thuyen ngoai xa:','Phung','Mien','My','Lien','A'),('Ai dat ten dong song:','NTuan','HPNTuong','VTPhung','Nam Cao','B'),('PC Nguyen Tuan:','Gian di','Tai hoa','Tru tinh','Hien thuc','B'),('Bai tho Song:','Xuan Quynh','HXHuong','Xuan Dieu','Huy Can','A'),('Nguoi lai do SD:','Truyen ngan','Tieu thuyet','Tuy but','Kich','C'),('Ba cu Tu Vo nhat:','Nhan hau','Phan khang','Kiem tinh','Me con','A'),('Hai dua tre doi lap:','AS-bong toi','Giau-ngheo','Tre-gia','Thi-que','A'),('PT van nghi luan:','Tu su','Mieu ta','Nghi luan','Bieu cam','C'),('Dat Nuoc:','NKDiem','NDThi','CLVien','To Huu','A'),('Noi qua thuoc:','An du','Hoan du','Noi qua','Nhan hoa','C'),('Rung xa nu:','NTThanh','NMChau','NHThiep','MVKhang','A'),('Hon Truong Ba:','NHTuong','LQVu','VTPhung','NDThi','B')]},
            'Lich su':{'t':50,'q':[('CM T8 ngay:','2/9/45','19/8/45','1/5/45','22/12/44','B'),('DBP ngay:','30/4/75','7/5/54','2/9/45','8/3/65','B'),('HD Paris nam:','1972','1973','1974','1975','B'),('Thong nhat:','2/9/45','7/5/54','30/4/75','22/12/76','C'),('LX ve tinh:','1955','1957','1961','1969','B'),('ASEAN lap:','1965','1967','1970','1975','B'),('CTTG2 ket thuc:','1944','1945','1946','1947','B'),('Can Vuong:','HNghi+TTT','PBChau','PCTrinh','NAQ','A'),('DCSVN lap:','3/2/30','19/5/30','2/9/30','1/5/30','A'),('Ap Bac 63:','Dong Nai','My Tho','Ben Tre','Long An','B'),('Dong minh My:','LX','TQ','Anh','Cuba','C'),('Berlin sup:','1987','1989','1991','1993','B'),('CT lanh KT:','LX tan ra 91','Berlin 89','My rut VN','TQ doi moi','A'),('Dong Du:','PCTrinh','PBChau','LVCan','HTKhang','B'),('LHQ lap:','1944','1945','1946','1950','B')]},
            'Sinh hoc':{'t':50,'q':[('ADN may mach:','1','2','3','4','B'),('Phien ma tao:','ADN','mARN','Protein','Lipit','B'),('Dot bien gen:','Bien doi NST','Bien doi gen','SL NST','Kieu hinh','B'),('Da boi NST:','2n','3n+','n','2n+1','B'),('CLTN tac dong:','Ca the','Quan the','Loai','HST','A'),('Pha sang QH o:','Chat nen','Tilacoit','Ti the','Nhan','B'),('Chuoi TT tu:','Tieu thu','San xuat','Phan giai','An thit','B'),('Aa la:','DH toi','DH tran','Di hop','KXD','C'),('PL doc lap:','Gen cung NST','Gen khac NST','Lien ket','Gioi tinh','B'),('Ho hap TB GD:','2','3','4','5','B'),('Nguyen phan:','Giam nhiem','Nguyen nhiem','TH protein','Phien ma','B'),('Ky sinh:','2 loi','1loi 1hai','2 hai','TT','B'),('Mang TB:','ADN','Lipit+protein','Tinh bot','Xen','B'),('Anticodon:','mARN','tARN','rARN','snARN','B'),('HST gom:','QX+SC','SV','MT','DV+TV','A')]},
            'Dia ly':{'t':50,'q':[('VN o:','DBA','DNA','Nam A','Tay A','B'),('DT VN:','231k','331k','431k','131k','B'),('Song dai nhat:','Hong','DNai','CuuLong','Ma','C'),('DB lon nhat:','sHong','sCuuLong','ThanhNghe','BTThien','B'),('Khi hau VN:','On doi','Nhiet doi GM','Xich dao','Han doi','B'),('TP lon nhat:','HN','HP','DN','TPHCM','D'),('Vung KT nhat:','DB sHong','Dong Nam Bo','TN','Bac TB','B'),('KS QNinh:','Dau mo','Than da','Sat','Vang','B'),('Rung nhiet doi:','Nhiet thap','Mua lon nhiet cao','Kho','Gio','B'),('Dan so 2024:','80tr','90tr','100tr','110tr','C'),('GDP cao nhat:','NN','CN','DV','KS','C'),('Bien Dong:','Bac','Tay','Dong','Nam','C'),('TN cay:','Lua','Ca phe','Mia','Bong','B'),('DB sHong dat:','Phu sa','Bazan','Cat','Phen','A'),('Nuoc DT lon nhat:','TQ','My','Canada','Nga','D')]},
            'GDCD':{'t':50,'q':[('PL la:','QT NN ban hanh','Dao duc','Tap quan','Quy uoc','A'),('BD truoc PL:','Lam gi dc','BD Q va NV','Chi nu','Giau nhieu Q','B'),('Hien phap:','Thuong','Co ban cao nhat','Nghi dinh','Thong tu','B'),('Q tu do KD:','HP','LDN','Ca HP+LDN','Khong co','C'),('Vi pham PL:','Trai DD','Trai PL co loi','Bat ky','Chi TP','B'),('Bau cu tu:','16','18','20','21','B'),('TNHS cho:','HC','Dan su','Toi pham','Ky luat','C'),('BD gioi:','Giong nhau','DV co hoi','Nu nhieu Q','Nam nhieu Q','B'),('BK xam pham:','Ko bi bat trai PL','Lam gi dc','Ko LD','Ko thue','A'),('Dao duc:','QT PL','QT le phai','Tap quan','Phong tuc','B'),('BM thu tin:','KT','Tu do CN','CT','VH','B'),('16t TNHS khi:','Bat ky','Rat/DB NG','Ko bao gio','Nhe','B'),('TH PL may HT:','2','3','4','5','C'),('QH co quan:','HP','TP','LP','KS','C'),('Muc dich PL:','Loi ich CN','Trat tu XH','Trung phat','Thue','B')]},
            'Tin hoc':{'t':50,'q':[('Nhi phan so:','8','10','2','16','C'),('1Byte=?Bit','4','8','16','32','B'),('CPU:','Process U','Processing Unit','Computer PU','Program U','B'),('Python in:','echo','printf','print()','write()','C'),('So nguyen Py:','float','str','int','bool','C'),('range(5):','4','5','6','10','B'),('RAM:','Chi doc','NN mat khi tat','Vinh vien','Ko ghi','B'),('Sort don gian:','Quick','Merge','Bubble','Heap','C'),('IPv4 bit:','16','32','64','128','B'),('SQL:','Simple','Structured','System','Standard','B'),('SQL truy van:','INSERT','UPDATE','SELECT','DELETE','C'),('OS mo:','Windows','macOS','Linux','iOS','C'),('Py list:','Bat bien','Mutable','Const','Ham','B'),('Bin search:','O(n)','O(n2)','O(logn)','O(1)','C'),('HTML:','Ngon ngu LT','Danh dau SVB','QTCSDL','OS','B')]},
        }
        for subj, data in _subs.items():
            title = 'De on thi TN THPT - ' + subj + ' 2026'
            cur = _c.execute("INSERT INTO exams(title,subject,description,time_limit,total_score,created_by,is_open,max_attempts,shuffle_questions,teacher_approved) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (title, subj, 'De on thi TN THPT 2026', data['t'], 10, _aid, 1, 999, 1, 1))
            _eid = cur.lastrowid
            for i, q in enumerate(data['q'], 1):
                _c.execute("INSERT INTO questions(exam_id,question_number,type,content,option_a,option_b,option_c,option_d,correct_answer,score) VALUES(?,?,?,?,?,?,?,?,?,?)",
                    (_eid, i, 'multiple_choice', q[0], q[1], q[2], q[3], q[4], q[5], 0.67))
        _c.commit()
        print("[AUTO-SEED] Da tao " + str(len(_subs)) + " de thi thanh cong!")
    else:
        print("[DB] Da co " + str(_cnt) + " de thi.")
    _c.close()
except Exception as e:
